...
void VeinsInetSampleApplication::processPacket(std::shared_ptr<inet::Packet> pk)
{
    ...
    if (!payload->getIsTrusted()) {

        EV_INFO << "Packet cannot be trusted! Dropping it" << endl;
        return;
    } else {
...
void VeinsInetSampleApplication::MaliciousAttack()
{
    ...
    bool isTrusted = getParentModule()->par("isTrusted").boolValue();

    auto payload = makeShared<VeinsInetSampleMessage>();
    payload->setChunkLength(B(100));
    payload->setIsTrusted(isTrusted);
...
void VeinsInetSampleApplication::sendAccident()
{
